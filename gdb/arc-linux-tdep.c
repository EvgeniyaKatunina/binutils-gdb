...
/*! Extracts result of the last condition checking from status32 register by
    condition code.
  ...
  @param[in] condition_code	It must be a condition code of the last
				...
				function may return wrong value.
  @param[in] status32		Status32 register' value.
  @return  The result of condition checking or -1 if given condition code is
	   wrong. */
  static int
arc_linux_is_condition_true (ULONGEST status32, int condition_code)
{
  int z_flag = status32 & ARC_REG_STATUS32_Z;
  int n_flag = status32 & ARC_REG_STATUS32_N;
  int c_flag = status32 & ARC_REG_STATUS32_C;
  int v_flag = status32 & ARC_REG_STATUS32_V;
...
/*! Checks result of brcc/bbit instruction condition by arguments and subopcode.
  ...
  @return  The result of condition checking or -1 if subopcode is not valid. */
  static int
arc_linux_is_brcc_taken(int subopcode, long arg1, long arg2)
{
  /* These variables will be used for unsigned comparison. */
  unsigned long uarg1 = arg1;
  unsigned long uarg2 = arg2;

  if (arc_debug)
  {
    fprintf_unfiltered(gdb_stdlog, "subopcode = %d, arg1 = %ld, arg2 = %ld,"
		       "uarg1 = %lu, uarg2 = %lu\n", subopcode, arg1, arg2,
		       uarg1, uarg2);
  }
...
/*! Returns register value by register number encoded in the instruction or long
    ...
    @param[in] byte_order	      Current machine' byte order.
  ...
  static long
arc_linux_read_from_reg_encod_in_instr(unsigned int register_number,
				       enum bfd_endian byte_order,
				       unsigned long word_after_instruction,
				       struct regcache *regcache)
{
  long register_value = 0;
  /* 62th register number indicates that operand is the long immediate value. */
...
arc_linux_next_pc (CORE_ADDR pc)
{
  ...
  int is_loop_enabled, is_target_taken;
  ...
  if (instr.isBranch)
    {
      ...
      if ((((instr.words[0] & 0x10000) >> 16) == 0x1) &&
	  ((instr._opcode == 0x1) || (instr._opcode == 0x1D)))
	{
	  is_brcc_or_bbit = TRUE;
	  ...
	  arg1_value = arc_linux_read_from_reg_encod_in_instr (arg1_reg,
							       byte_order,
							       instr.words[1],
							       regcache);
	  arg2_value = arc_linux_read_from_reg_encod_in_instr (arg2_reg,
							       byte_order,
							       instr.words[1],
							       regcache);
	  is_branch_taken = arc_linux_is_brcc_taken (instr_subopcode,
      ...
      if (!is_brcc_or_bbit)
	{
	  ...
	  is_branch_lpcc = (instr_subopcode == 0x28) && (instr._opcode == 0x4);
          regcache_cooked_read_unsigned (regcache, gdbarch_ps_regnum (gdbarch),
				&status32);
	  is_branch_taken = arc_linux_is_condition_true (status32, instr._cond);
	}
      if (arc_debug)
	{
	  fprintf_unfiltered (gdb_stdlog,
	 ...
	 is an another branch or jump and branch is taken. */
